...
class MarcaController {
  async GetAll(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error al obtener las marcas", error });
    }
  }
  ...
  async GetById(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error al obtener la marca", error });
    }
  }
  ...
  async Save(req, res) {
    try {
      ...
      if (!nombre || nombre.trim() === "") {
        return res
          .status(400)
          .json({ message: "Nombre de la marca es requerido" });
      }
      ...
      return res
        .status(500)
        .json({ message: "Error al crear la marca y subcategorías", error });
    }
  }
  ...
  async SavesAll(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error al crear las marcas", error });
    }
  }
  ...
  async Belongs(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({
          message: "Error al obtener las categorías de la marca",
          error,
        });
    }
  }
  ...
  async Update(req, res) {
    try {
      ...
      if (subcategorias && subcategorias.length > 0) {
        ...
        const subcategoriasExistentesMap = subcategoriasExistentes.reduce(
          (map, subcategoria) => {
        ...
        const subcategoriasActualizadas = await Promise.all(
          subcategorias.map(async (subcategoria) => {
            if (
              subcategoria.id &&
              subcategoriasExistentesMap[subcategoria.id]
            ) {
        ...
        marca.CategoriaMarcas = subcategoriasActualizadas.filter(
      ...
      return res
        .status(500)
        .json({
          message: "Error al actualizar la marca y subcategorías",
          error,
        });
    }
  }
  ...
  async Delete(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Error al eliminar la marca", error });
    }
  }
}
